            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <form action="{{ route('booking.storePlotRooms', $booking->id) }}" method="POST">
                        ...
                        <table id="roomslist" class="table table-bordered">
                            <thead>
                                <tr>
                                    <th>Pilih</th>
                                    <th>Unit</th>
                            ...
                            <tbody>
                                ...
                                @foreach ($data['rooms'] as $room)
                                    ...
                                    <tr>
                                        <td>
                                            <input type="checkbox" name="rooms[]" value="{{ $room->id }}" class="room-checkbox"
                                            data-capacity="{{ $room->kapasitas ?? 0 }}"
                                            @if ($isSelected) checked @endif>
                                        </td>
                                        <td>{{ $room->branch->name }}</td>
                                        <td>{{ $room->nama }}</td>
                                        <td>{{ $room->kapasitas }}</td>
                                        <td><span class="badge {{ $room->status == 'available' ? 'badge-soft-success' : 'badge-soft-danger' }}">{{ $room->status }}</span></td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
